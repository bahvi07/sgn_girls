...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (sameAddressCheckbox && localAddressField) {
    sameAddressCheckbox.addEventListener('change', function() {
      localAddressField.style.display = this.checked ? 'none' : 'block';
    });
    
    // Initialize the visibility based on checkbox state
    localAddressField.style.display = sameAddressCheckbox.checked ? 'none' : 'block';
  }
  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', async (e) => {
      ...
      // Get required fields
      const applicantName = form.querySelector('[name="applicant_name_english"]');
      const motherName = form.querySelector('[name="mother_name_english"]');
      const dob = form.querySelector('[name="date_of_birth"]');
      const category = form.querySelector('[name="category"]');
      const aadhar = form.querySelector('[name="aadhar_number"]');
      const permAddress = form.querySelector('[name="permanent_address"]');
      const phone = form.querySelector('[name="mobile_number"]');
      const photo = form.querySelector('[name="applicant_photo"]');
